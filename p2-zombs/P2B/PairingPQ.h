...
public:
    ...
    PairingPQ(const PairingPQ &other) :
        ...
        std::deque<Node*> toDo;
        toDo.push_back(other.root);
        while(!toDo.empty()){
            Node* ptr = toDo.front();
            if(ptr->child != nullptr)
                toDo.push_back(ptr->child);
            if(ptr->sibling != nullptr)
                toDo.push_back(ptr->sibling);
            toDo.pop_front();
            push(ptr->getElt());
        }
    } // PairingPQ()
    ...
    void updateElt(Node* node, const TYPE &new_value) {
        // TODO: Implement this function
        node->elt = new_value;
        if(this->compare(node->parent->getElt(), new_value)){
            if(node->parent->child != node){
                Node* ptr = node->parent->child;
                while(ptr->sibling != node){
                    ptr = ptr->sibling;
                }
...
private:
    ...
    //pa nad pb must have no parents and sibling
